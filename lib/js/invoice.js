...
function deleteProduct() {
	...
	var respuesta='eliminarDefinitivo';
	$.ajax({
		url:'actualizarInventario.php',
		type:'POST',
		data:{idCotizacion,respuesta},
...
function imprimir() {
	...
	var serieX= document.getElementById('opcionSerie').selectedIndex;
    var serieY = document.getElementById('opcionSerie').options;
    var estatusX=  document.getElementById('opcionEstatus').selectedIndex;
    var estatusY = document.getElementById('opcionEstatus').options;
    var pagoX=  document.getElementById('opcionPago').selectedIndex;
    var pagoY = document.getElementById('opcionPago').options;
	...
	//var respuesta='actualizarSimple';
   ...
   /* $.ajax({
        url:'../actualizarInventario.php',
        type:'POST',
        data:{facturado,folio,respuesta,credito,pago,estatus,descuento},
        success:function(respuesta){
            alert(respuesta);
	...
	window.open("plantillaActualizda.php?facturado=" +facturado+ "&descuento="+ descuento +"&folio="+folio+"&estatus="+estatus+
	"&pago="+pago+"&credito="+credito+"&serie="+serie+" "); 
...
function tablaParametros(cantidad,descripcion,precioUnitario,subtotal,iva,total,accion,id,cantidadDescontar,metros,tramos){
	...
	$.ajax({
		url:'tablaTemporal.php',
		type:'POST',
		data:{cantidad,descripcion,precioUnitario,subtotal,iva,total,accion,id,cantidadDescontar,metros,tramos,eliminado,folio
		,idC
		,estatus,
		facturado,
		pago,
...
/*<?php  
                          $query1=$dbConexion->query("select * from cotizacionTemporal where eliminado='si'");
                                 foreach($query1 as $que){
                                  
                          ?>
                          
                           <tr>
                           <td> <?php echo $que['cantidad']; ?> </td>
                           <td> <?php echo $que['cantidad']; ?> </td>
                           <td> <?php echo $que['cantidad']; ?> </td>
                           <td> <?php echo $que['cantidad']; ?> </td>
                           <td> <?php echo $que['cantidad']; ?> </td>
